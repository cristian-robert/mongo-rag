...
from .filter_config import FilterConfig, FilterMetadata
from .content_filter import create_filter
...
class DocumentIngestionPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            filter_config: Optional content filter configuration for selective ingestion
        """
        self.config = config
        ...
        # Initialize content filter if config provided
        self.content_filter = create_filter(filter_config) if filter_config else None
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        if self.content_filter and docling_doc:
            logger.info(f"Applying content filter to {document_title}")
            try:
                docling_doc, filter_metadata = await self.content_filter.filter_document(
                    docling_doc=docling_doc,
                    file_path=file_path
                )
                # Re-export markdown from filtered document
                document_content = docling_doc.export_to_markdown()
                # Re-extract metadata from filtered content
                document_metadata = self._extract_document_metadata(document_content, file_path)
                # Add filter metadata to document metadata
                if filter_metadata:
                    document_metadata["filter_metadata"] = {
                        "sections_removed": filter_metadata.sections_removed,
                        "pages_removed": filter_metadata.pages_removed,
                        "blocks_removed": filter_metadata.blocks_removed,
                        "filter_config_used": filter_metadata.filter_config_used
                    }
                logger.info(
                    f"Filtering complete: {filter_metadata.blocks_removed} blocks removed, "
                    f"{len(filter_metadata.sections_removed)} sections excluded"
                )
            except Exception as e:
                logger.error(f"Content filtering failed for {document_title}: {e}")
                logger.warning("Continuing with unfiltered document")
...
def load_filter_config(config_path: str) -> Optional[FilterConfig]:
    """
    Load filter configuration from JSON file.

    Args:
        config_path: Path to filter config JSON file

    Returns:
        FilterConfig instance or None if loading fails

    Raises:
        FileNotFoundError: If config file doesn't exist
        json.JSONDecodeError: If config file is not valid JSON
        ValueError: If config doesn't match FilterConfig schema
    """
    ...
    try:
        ...
        # Validate and instantiate FilterConfig
        filter_config = FilterConfig(**config_dict)

        logger.info(
            f"Loaded filter config for {filter_config.subject}: {filter_config.book}"
        )
        logger.info(
            f"  Exclude sections: {len(filter_config.exclude_sections)}, "
            f"pages: {len(filter_config.exclude_pages)}, "
            f"patterns: {len(filter_config.exclude_patterns)}"
        )

        return filter_config

    except json.JSONDecodeError as e:
        logger.error(f"Invalid JSON in filter config file: {e}")
        raise
    except TypeError as e:
        logger.error(f"Filter config doesn't match expected schema: {e}")
        raise ValueError(f"Invalid filter config schema: {e}")
...
async def main():
    ...
    parser.add_argument("--filter-config", "-f", type=str, help="Path to filter config JSON file (optional)")
    ...
    if args.filter_config:
        try:
            ...
            logger.warning("Continuing without content filtering")
